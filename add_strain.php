...
// Insert new strain if not existing
    ...
    if (mysqli_num_rows($check_query) < 1) {
            ...
            if (isset($_SESSION['user_id']) && !empty($_SESSION['user_id'])) {
                if ($stmt_strain->bind_param("ssis", $strain, $comment, $creator, $current_date)) {
                    if ($stmt_strain->execute()) {
                        ...
                        header("Location: http://" . $_SERVER['HTTP_HOST'] . rtrim(dirname($_SERVER['PHP_SELF']), '/\\') . "/" . "new_insert.php?content=new_strain");
                        exit();
                    } else {
        ...
        $_SESSION['existing'] = "<div class = 'existing'>The entered strain already exists! Please enter a new one.</div>";
        mysqli_close($link) or die("Could not close database connection");
        header("Location: http://" . $_SERVER['HTTP_HOST'] . rtrim(dirname($_SERVER['PHP_SELF']), '/\\') . "/" . "new_insert.php?content=new_strain");
        exit();
    }
