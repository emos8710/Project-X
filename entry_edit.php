...
if($loggedin && $active && $admin) {
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		// Remove insert
			...
			if ($result = mysqli_query($link, $remove_sql)) {
				if ($result = mysqli_query($link, $move_sql)) {
					$update_msg = "Successfully removed insert.";
				} else {
					$iserror = TRUE;
				$update_msg = "Failed to move inserts. ".mysqli_error($link);
				goto errorTime;
				}
			} else {
				$iserror = TRUE;
	...
	<ul>
		<!-- Edit registry ID -->
		<li>Registry ID <?php echo $entry_info["biobrick"];
		...
		<!-- Edit strain -->
		<li>Strain <?php echo $entry_info["sname"];
		...
		<!-- Edit backbone -->
		<li>Backbone <?php echo $entry_info["bname"];
		...
		<!-- Edit year created -->
		<li>Year created <?php echo $entry_info["year_created"];
